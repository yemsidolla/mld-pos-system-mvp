...
- Reference costs are dashboard records protected by catalog management and cost visibility permissions.
- Creating or editing a reference cost creates `AuditLog.Action.COST_CHANGE`.
- Product default cost is fallback planning data.
- Stock-in actual unit cost records what was paid for one received batch.
- Stock-in landed unit cost optionally includes shipping, handling, import, or other landed cost for one received batch.
- Stock-in is also cost-visibility gated because it exposes actual and landed cost fields.

Evidence:

- `catalog.models.SupplierProductCost`
- `catalog.views.supplier_product_cost_list_view()`
- `inventory.views.stock_in_view()`

## Promotion Rules

- Only Admin users can create or edit promotions.
- Promotions can apply to a product or a category.
- Dashboard promotion forms require choosing either a product or a category, not both.
